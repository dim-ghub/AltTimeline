...
void Player::knockback(shared_ptr<Entity> source, float dmg, double xd, double zd)
{
    float totalKnockbackResistance = getAttribute(SharedMonsterAttributes::KNOCKBACK_RESISTANCE)->getValue();

    for (int i = 0; i < 4; i++)
    {
        shared_ptr<ItemInstance> armor = inventory->getArmor(i);
        if (armor != nullptr && armor->getItem() != nullptr)
        {
            ArmorItem *armorItem = dynamic_cast<ArmorItem *>(armor->getItem());
            if (armorItem != nullptr && armorItem->getMaterial() == ArmorItem::ArmorMaterial::NETHERRITE)
            {
                totalKnockbackResistance += 0.1f;
            }
        }
    }

    if (random->nextDouble() < totalKnockbackResistance)
    {
        return;
    }
...
getAttributes()->registerAttribute(SharedMonsterAttributes::KNOCKBACK_RESISTANCE)->setBaseValue(knockbackResistance);
}
